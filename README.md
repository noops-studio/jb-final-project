...
- **GitHub Actions** — builds the image, pushes it to **GHCR**, and bumps the Helm tag.
- **ArgoCD (GitOps)** — watches `my-monitor/` on `dev` and deploys every change automatically.

```
push code ──▶ GitHub Actions ──▶ ghcr.io/.../uptime-kuma:sha-xxxx
                     │
                     └─ bump my-monitor/values.yaml ──▶ commit to dev
                                                              │
...
2. Run the workflow once manually: **Actions ▸ build-and-deploy ▸ Run workflow ▸ branch `dev`**.
   - It builds `ghcr.io/noops-studio/uptime-kuma:sha-<short>` and commits that real tag into
     `my-monitor/values.yaml` (replacing the `preflight` placeholder).
